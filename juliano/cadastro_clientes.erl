...
adicionar_cadastro(Clientes) ->
    ...
    CpfAdicionar = list_to_integer(utilitario_clientes:pede_tira_n("Digite o CPF do Cliente")),
    ...
    CepAdicionar = list_to_integer(utilitario_clientes:pede_tira_n("Digite o CEP do Cliente")),
    EmailAdicionar = utilitario_clientes:pede_tira_n("Digite o email do Cliente"),
    Email2Adicionar = utilitario_clientes:pede_tira_n("Digite o email secundario do Cliente"),
    TelefoneAdicionar = utilitario_clientes:pede_tira_n("Digite o telefone do Cliente"),
    Telefone2Adicionar = utilitario_clientes:pede_tira_n("Digite o telefone secundario do Cliente"),
...
excluir_clientes(Clientes, Id) ->
    F = fun(Cliente) ->
            NumeroId = list_to_integer(utilitario_clientes:get_value(id, Cliente),
            io:format("NomeEmpreendimento = ~p,  Nome = ~p~n", [NumeroId, Id]),
            (NumeroId /= Id )
        end,
...
editar_clientes([], _Id, Acc) ->
    Acc;
editar_clientes([CabCliente|Clientes], Id, Acc) ->
    IdCliente = list_to_integer(utilitario_clientes:get_value(id, CabCliente)),
    IdNovo = case Id == IdCliente of
        true -> update_clientes(CabCliente);
        _ -> CabCliente
    end,
    editar_clientes(Clientes, Id, [IdNovo | Acc]).

update_clientes(Cliente) ->
    io:format("############ Você está Editando ##############~n"),
    ...
    CpfDigitado = list_to_integer(utilitario_clientes:pede_tira_n(["Novo CPF: (", utilitario_clientes:get_value(cpf, Cliente), "): " ])),
    ...
    CepDigitado = list_to_integer(utilitario_clientes:pede_tira_n(["Novo CEP: (", utilitario_clientes:get_value(cep, Cliente), "): " ])),
    EmailDigitado = utilitario_clientes:pede_tira_n(["Novo Email: (", utilitario_clientes:get_value(email, Cliente), "): " ]),
    Email2Digitado = utilitario_clientes:pede_tira_n(["Novo Email Secundario: (", utilitario_clientes:get_value(email2, Cliente), "): " ]),
    TelefoneDigitado = list_to_integer(utilitario_clientes:pede_tira_n(["Novo Telefone: (",utilitario_clientes:get_value(telefone, Cliente),"): " ])),
    Telefone2Digitado = list_to_integer(utilitario_clientes:pede_tira_n(["Novo Telefone: (",utilitario_clientes:get_value(telefone2, Cliente), "): " ])),
